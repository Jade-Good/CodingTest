...
int main(void) {
	...
	for (int i = 0; i < K; i++)
	{
		...
		while (!q.empty()) {
			...
			if (now - 1 >= 0) {		// 왼쪽 톱니바퀴 존재
				if (flag[now - 1] == 0 && top[now - 1][2] != left) { // 왼쪽 톱니바퀴가 아직 돌아가지 않았고, 접점의 극이 반대
					q.push(now - 1);
					flag[now - 1] = flag[now] * -1;
				}
			}
			if (now + 1 <= 3) {		// 오른쪽 톱니바퀴 존재
				if (flag[now + 1] == 0 && top[now + 1][6] != right) { // 오른쪽 톱니바퀴가 아직 돌아가지 않았고, 접점의 극이 반대
					q.push(now + 1);
					flag[now + 1] = flag[now] * -1;
				}
			}
