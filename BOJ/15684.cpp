...
bool LineCheck() {
	...
	for (int i = 1, n; i < N; i++) // start N
	{
		n = i;
		for (int h = 1; h <= H; h++)// ladder down
		{
			if (ladder[h][n]) n = n + 1;
			else if (ladder[h][n - 1]) n = n - 1;
		} 
		if (n != i) {
			// cout << "false!!" << endl;
			return false;
...
void plus_line(int count_line, int idx) {

	if (count_line > 3) return;

	if (LineCheck()) {
		answer = min(answer, count_line);
		return;
	}
...
int main(void) {
	...
	{
		cout << "hubo : " << hubo[i][0] << " " << hubo[i][1] << endl;
	}*/
